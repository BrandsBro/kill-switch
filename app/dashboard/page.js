      ...
      <div className="relative w-full max-w-3xl rounded-3xl overflow-hidden"
        ...
        <div className="flex items-center justify-between px-5 py-3 sticky top-0 z-10"
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs font-mono ml-2 text-zinc-600">{kill['URL']?.substring(0,45)}...</span>
          </div>
...
export default function Dashboard() {
  ...
  const toggleSwitch = async (kill, i) => {
    ...
    formData.append('data', JSON.stringify({ action, url: kill['URL'] }))
    try {
      await fetch(scriptUrl, { method: 'POST', body: formData, mode: 'no-cors' })
      setKills(prev => prev.map(k =>
        k === kill ? { ...k, Status: isOn ? 'OFF' : 'ON' } : k
      ))
    } catch {}
    setSwitching(null)
  }
  ...
  const stats = {
    ...
    today: kills.filter(k => { try { return new Date(k['Killed At']).toDateString() === new Date().toDateString() } catch { return false } }).length,
      ...
      <div className="max-w-5xl mx-auto relative z-10">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-6">
          ...
          <div className="rounded-2xl p-5" style={{ background: 'rgba(220,38,38,0.06)', border: '1px solid rgba(220,38,38,0.15)' }}>
            <p className="text-xs text-red-600 uppercase tracking-widest font-bold mb-1">🔴 Active Kills</p>
                ...
                <div key={i} className="rounded-2xl p-5 transition-all duration-300"
                  ...
                  <div className="flex items-center justify-between gap-4">
                    <div className="flex-1 min-w-0">
                      ...
                      {kill['Message'] && (
                        <p className="text-zinc-600 text-xs mt-1">💬 {kill['Message']}</p>
                      )}
                    </div>
